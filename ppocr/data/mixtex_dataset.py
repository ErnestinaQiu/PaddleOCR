...
class MixTexDataSet(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        target = self.tokenizer.tokenizer(target_text, padding="max_length", max_length=self.max_seq_len, truncation=True).input_ids
        labels = [label if label != self.tokenizer.tokenizer.pad_token_id else -100 for label in target]
